...
void main() {
  ...
  test('periodic movement updates the filter with the AMDF estimate', () async {
    ...
    _emitSamples(
      sensor,
      (index) => 9.81 + 2 * sin(2 * pi * 180 / 60 * index / sampleRate),
    );
    final state = await loaded;

    expect(state.rawBpm, closeTo(180, 1));
    expect(state.bpm, greaterThan(150));
    expect(state.bpm, lessThan(state.rawBpm));
    expect(state.confidence, greaterThan(0.8));
...
void _emitSamples(_FakeSensorService sensor, double Function(int index) xAt) {
  ...
  for (var index = 0; index < windowSize; index++) {
    sensor.add(
      AccelerometerEvent(
        ...
        0,
        0,
        start.add(Duration(milliseconds: index * 20)),
      ),
    );
  }
}
